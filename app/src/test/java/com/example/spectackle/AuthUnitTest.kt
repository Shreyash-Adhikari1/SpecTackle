package com.example.spectackle



    import com.example.spectackle.repository.AuthRepoImpl
    ...
    class AuthUnitTest {
        ...
        fun testSignUp_Successful() {
            ...
            var expectedUserId="Initial User ID"// Define the initial value

            // Mocking task to simulate successful registration
            `when`(mockTask.isSuccessful).thenReturn(true)
            `when`(mockAuth.createUserWithEmailAndPassword(any(), any())) // <-- FIXED HERE
                .thenReturn(mockTask)

            val mockUser = mock<FirebaseUser>()
            `when`(mockAuth.currentUser).thenReturn(mockUser)
            `when`(mockUser.uid).thenReturn("mockedUserId")

            // Define a callback that updates the expectedResult
            val callback = { success: Boolean, message: String? , userId:String ->
                expectedResult = message ?:"Unknown message"
            ...
            assertEquals("mockedUserId",expectedUserId)
        }
